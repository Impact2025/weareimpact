 ...
 *  2. herinnering naar de klant (alleen als reminders_enabled, max 1× per 3 dagen)
 *  3. digest naar Vincent, alleen als er iets aandacht vraagt
...
async function run(request: NextRequest) {
  ...
  try {
    ...
    const settings = await sql`
      SELECT slug, name, reminders_enabled, last_reminder_at
    ...
    for (const o of overviews) {
      ...
      const contact = await sql`
        ...
        WHERE project_slug = ${o.slug} AND audience = 'klant'
      ...
      if (dry) {
        remindersSent.push(`${o.name} (dry-run → ${contact[0].email})`);
        continue;
      }
      const { url } = await createMagicLink(o.slug, 'klant', contact[0].email as string);
      const mail = generateLaunchClientReminderEmail({ projectName: o.name, portalUrl: url, tasks: o.clientTasksOpen });
      const res = await sendEmail({ to: contact[0].email as string, ...mail });
      if (res.success) {
